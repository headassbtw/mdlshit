...
int Conversion::ReadHeader(FileInfo info) {
  ...
  //Stream.seek(mdl.mdlhdr.boneindex);
  OutStream.seek(v53Hdr.boneindex);
  //int bonecount = mdl.mdlhdr.numbones;
...
//          int stairs = bytesAddedToAnims + bytesAddedToSeqs + (28 * bonecount) - (28 * i) + bytesAddedToTextures + bytesAddedToIkChains + bytesAddedToAnimData + bytesAddedToActMods + textureFiller + strFiller + bytesAddedToRuiMesh;
  ...
  //int anim_filler_dest = mdl.mdlhdr.localanimindex - Stream.Position();
  //filler(&Stream, &OutStream, anim_filler_dest);

  //Stream.seek(mdl.mdlhdr.localanimindex);
  ...
  { 
      ...
      for (int i = 0; i < mdl.mdlhdr.numlocalanim; i++)
      {
          ...
          //int strStartPos = mdl.mdlhdr.localanimindex + 100 * i;
          //int PISS2 = -((8 * (mdl.mdlhdr.numlocalanim - i))) + hdrBytesAnimDescAdd[i] + secHdrBytesAnimDescAdd[i] + ikRuleStairsPerAnim[i] - 4;
          //int PISS3 = -((8 * (mdl.mdlhdr.numlocalanim - i))) + hdrBytesAnimDescAdd[i] - 4;
          OutStream.seek(startPos + animdescs[i].animindex - 4);


          //int strPos = mdl.mdlhdr.localanimindex + 100 * i;
          ...
          if (animDesc.sectionindex > 0) //Gosh do I hate how this is setup. - Liberty
          {
...
//                 Stream.seek(strPos + animOffset);
          ...
          if (animDesc.numikrules > 0)
          {
              ...
              for (int j = 0; j < animDesc.numikrules; j++)
              {
                  ...
                  //if(ikRules[ikRuleNum].compressedikerrorindex > 0) numOfCompressedIkErrors++;
                  //if(ikRules[ikRuleNum].ikerrorindex > 0) numOfIkErrors++;
  ...
  for (int i = 0; i < mdl.mdlhdr.numlocalseq; i++)
  {
      ...
      if (seqDesc.numautolayers > 0)
      {
          ...
          {
              //OutStream.seek(outPos + seqDesc.posekeyindex + stairs + 4 * numOfActMods + 24 * j);
      ...
      if (seqDesc.numevents > 0)
      {
          ...
          {
              //OutStream.seek(outPos + seqDesc.eventindex + stairs + 4 * numOfActMods + 80 * j);
      ...
      //    filler(&Stream, &OutStream, 4 * Initial_Header->numbones);
      //}
      //numOfActMods += seqDesc.numactivitymodifiers;
  ...
  if (mdl.mdlhdr.numlocalnodes > 0)
  {
      ...
      //Stream.seek(mdl.mdlhdr.localnodenameindex);
  ...
  if (mdl.mdlhdr.numbodyparts > 0)
  {
      ...
      for (int i = 0; i < mdl.mdlhdr.numbodyparts; i++) {
          ...
          if (info.disable_bodyparts) {
              ...
              //if (mdl.bodyparts[i].nummodels > 0) numOfModels += mdl.bodyparts[i].nummodels;
  ...
  if (mdl.models.size() > 0)
  {
      ...
      //OutStream.seek(Dest_Header->bodypartindex + 16 * mdl.bodyparts.size());
  ...
  //Stream.seek(mdl.mdlhdr.includemodelindex);
...
#pragma region texture conversion
  ...
  //Stream.seek(mdl.mdlhdr.textureindex);
...
#pragma endregion
  ...
  //Stream.seek(Initial_Header->keyvalueindex);
  ...
  Stream.seek(mdl.mdlsubhdr.sznameindex + 407);
  //mstudiolinearbone_t_v49 linearBone; Stream.Read(&linearBone);
  //mstudiolinearbonedata_t_v49 linearBoneData; Stream.Read(&linearBoneData, Initial_Header->numbones);
